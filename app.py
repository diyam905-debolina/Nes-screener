...
st.caption("Filters: EMA 50/200 · RSI 55–70 · MACD · ADX · Price ₹50–₹5000")
...
STOCKS = [
    ...
    "ONGC.NS","JSWSTEEL.NS","TATAMOTORS.NS","TATASTEEL.NS","ADANIENT.NS",
    "BAJFINANCE.NS","BAJAJFINSV.NS","HCLTECH.NS","TECHM.NS","DIVISLAB.NS",
    "DRREDDY.NS","CIPLA.NS","APOLLOHOSP.NS","EICHERMOT.NS","HEROMOTOCO.NS",
    "BRITANNIA.NS","DABUR.NS","GODREJCP.NS","MARICO.NS","COLPAL.NS",
    "HAVELLS.NS","POLYCAB.NS","DIXON.NS","VOLTAS.NS","BLUESTARCO.NS",
    "PERSISTENT.NS","MPHASIS.NS","COFORGE.NS","LTIM.NS","KPITTECH.NS",
    "HAL.NS","BEL.NS","BHEL.NS","IRCTC.NS","RVNL.NS",
    "IRFC.NS","HUDCO.NS","NBCC.NS","COCHINSHIP.NS","MAZAGON.NS",
    "SUZLON.NS","TATAPOWER.NS","ADANIGREEN.NS","CESC.NS","TORNTPOWER.NS",
    "ZOMATO.NS","NYKAA.NS","DELHIVERY.NS","PAYTM.NS","POLICYBZR.NS",
    "CDSL.NS","CAMS.NS","ANGELONE.NS","HDFCAMC.NS","ABSLAMC.NS",
    "BAJAJHFL.NS","MUTHOOTFIN.NS","CHOLAFIN.NS","MANAPPURAM.NS","IIFL.NS",
    "APOLLOTYRE.NS","BALKRISIND.NS","CEAT.NS","MRF.NS","TIINDIA.NS",
    "AUROPHARMA.NS","ALKEM.NS","IPCALAB.NS","NATCOPHARM.NS","GLAND.NS",
    "TRENT.NS","DMART.NS","ABFRL.NS","PAGEIND.NS","KALYANKJIL.NS"
]
    ...
    for i, ticker in enumerate(STOCKS):
        try:
            ...
            if (latest_close > latest_ema50 and
                ...
                45 <= latest_rsi <= 75 and
                latest_macd > latest_signal and
                latest_adx > 15 and
                latest_vol >= 0.5 * latest_avg_vol and
                week_perf >= -2 and
                50 <= latest_close <= 10000 and
                dist_52w <= 25):
                if latest_rsi > 70:
                    signal = "⚠️ Near Exit"
                ...
                results.append({
                    ...
                    "MACD": "✅ Above" if latest_macd > latest_signal else "❌ Below",
                    "ADX": round(latest_adx, 1),
                    "Vol Ratio": round(latest_vol / latest_avg_vol, 2),
                    "52W Dist %": round(dist_52w, 1),
                })
        except:
            pass
...
def plot_stock_chart(ticker, symbol):
    ...
    fig2.update_layout(title=f"{symbol} — RSI 14",
    ...
    fig3 = go.Figure()
    fig3.add_trace(go.Bar(x=df.index, y=macd_hist, name="Histogram",
    ...
    fig3.add_trace(go.Scatter(x=df.index, y=signal_line, name="Signal",
        line=dict(color="#EF9F27", width=1.5)))
    fig3.update_layout(title=f"{symbol} — MACD",
        height=250, plot_bgcolor="white", paper_bgcolor="white")
    ...
    fig4.update_layout(title=f"{symbol} — ADX Trend Strength",
        height=250, plot_bgcolor="white", paper_bgcolor="white")
    ...
    current_signal_val = float(signal_line.iloc[-1])
    st.markdown("**Trend Summary:**")
    if current_macd > current_signal_val and current_adx > 25 and current_rsi < 70:
        st.success("✅ Strong uptrend — MACD positive, ADX strong, RSI healthy. Good zone to study for entry.")
    elif current_rsi > 70:
        st.warning("⚠️ Stock is overbought — RSI above 70. Consider booking partial profit.")
    elif current_adx < 20:
...
if not df.empty:
    ...
    selected = st.selectbox("Choose a stock to analyse:", options=df["Symbol"].tolist())
    if selected:
        ticker_row = df[df["Symbol"] == selected].iloc[0]
        plot_stock_chart(ticker_row["Ticker"], selected)
    st.info("""
    ✅ Strong Buy → MACD up + ADX strong + RSI healthy. Good to study for entry.
    👀 Watch → Keep on radar. Not ready yet.
    ⚠️ Near Exit → RSI above 70. Consider booking profit.
    """)
else:
    st.warning("No stocks matched today's filters. Tap Refresh Data to try again.")
